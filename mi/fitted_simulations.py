import numpy as np
import torch
from envs import Badham2017, Devraj2022
import argparse
...
def simulate_metalearner_with_fitted_coefficients(env=None, model_path=None, participant=0, beta=1., epsilon=0., method='soft_sigmoid', device='cpu', **kwargs):
    ...
    # load model
    if method == 'bounded':
        model = TransformerDecoderClassification(num_input=env.num_dims, num_output=env.num_choices, num_hidden=256, num_layers=6, d_model=64, num_head=8, max_steps=650, loss='bce', device=device).to(device)
...
def simulate_metalearner(args):
    ...
    def objective(env=None, model_name=None, method='soft_sigmoid', num_runs=1, **task_features):
        ...
        if args.method == 'bounded':
            parameters_path = f"{SYS_PATH}/categorisation/data/model_comparison/task=devraj2022_experiment=1_source=claude_condition=unknown_loss=nll_paired=True_method=bounded_optimizer=grid_search_numiters=5.npz"
        else:
        ...
        for idx, participant in enumerate(participants):
            if args.method == 'bounded':
                beta = parameters['beta'][idx]
                ess = parameters['ess'][idx]
                epsilon = 0.0
                model_path = f"{SYS_PATH}/categorisation/trained_models/env=claude_generated_tasks_paramsNA_dim6_data500_tasks12910_pversion5_stage2_model=transformer_num_episodes500000_num_hidden=256_lr0.0003_num_layers=6_d_model=64_num_head=8_noise0.0_shuffleTrue_lossbce_ess{float(ess)}_run=2_regall_essinit0.0_annealed.pt"
            else:
                epsilon = parameters['betas'][idx] if args.method == 'eps_greedy' else 0.
                beta = parameters['betas'][idx] if args.method == 'soft_sigmoid' else 1.
